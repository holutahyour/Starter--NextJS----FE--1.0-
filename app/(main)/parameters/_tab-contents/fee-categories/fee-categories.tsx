...
function FeeCategories() {
  ...
  const [data, setData] = useState<IFeeCategory[]>([]);
  ...
  useEffect(() => {
    if (currentTab === "fee-categories") {
      const fetchData = async () => {
        setTableLoader(true);
        try {
          const response = await getFeeCategoriesData({
  ...
  const handleImport = (acceptedFiles: File[]) => {
    ...
    if (file) {
      processExcelFile(file, (data) => {
        const promise = apiHandler.feeCategory
          .import(data)
  ...
  return (
    <>
      <AppDataTable
        ...
        filter="itemName"
        filterPlaceholder="Filter fee categories..."
        pageCount={pageCount}
...
export async function getFeeCategoriesData(params: { page: number; pageSize: number }) {
  const { page, pageSize } = params;
  const Info = await apiHandler.feeCategory.list({
    page,
    pageSize,
  });
  return Info;
